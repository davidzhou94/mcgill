...
let lazy_church f =
  let rec inner f g =
    { hd = (fun x -> g x) ;
      tl = Susp ( fun () -> inner f (fun x -> f (g x)) )
    }
  in
inner f (fun x -> x);;
...
let rec l_tree_map f t =
  { nd = f (t.nd) ;
    ch = Susp ( fun () -> l_tree_map' f (force t.ch) )
  }
and l_tree_map' f t = match t with
  | Empty      -> Empty
  | NonEmpty l -> NonEmpty (List.map (fun x -> l_tree_map f x) l)
...
let rec l_tree_trav t =
  t.nd:: l_tree_trav' (force t.ch)
and l_tree_trav' t = match t with
  | Empty -> []
  | NonEmpty l -> List.flatten (List.map (fun c -> l_tree_trav c) l)
...
(*
let inception t =
  let rec inception' t (c:(unit -> 'a fin_list)) =
    { lhd = t.nd ;
      ltl = Susp ( fun () -> match (force t.ch) with
                     | Empty -> c ()
                     | NonEmpty l -> match l with
                         | x::xs -> Some (inception x (fun () -> List.map (fun t -> inception t c) xs ))
                         | []    -> c () )
    }
  in
inception' t ( fun () -> None )
*)

(*
and inception' t = match t with
  | Empty -> None
  | NonEmpty l -> incept'' l
and incept'' l = match l with
  | x::xs -> Some ( inception x )
  | [] -> None

let test t = inception t (fun x -> match x with None -> None | Some l -> l)*)

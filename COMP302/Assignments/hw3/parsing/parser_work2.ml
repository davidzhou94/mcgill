break the list up into two lists where the seper
...
let rec list_after_token toklist exptoken =

let rec list_before_token toklist exptoken =

let rec token_exists toklist exptoken =

(* Lexer.token list -> ( Lexer.token list -> exp -> exp ) -> exp *)
...
and parseSum toklist sc = match toklist with
  | [ ] -> raise (Error "Error: Incomplete expression")
  | l -> if (token_exists l L.PLUS) then
      Sum ( parseProd (list_before_token l L.PLUS) sc, parseSum (list_after_token l L.PLUS) sc)
    else
      parseProd l sc


(* probably not continuations below *)

let rec parseExp toklist sc = match toklist with
  | [ ] -> raise (Error "Error: Incomplete expression")
  | l -> parseSum l (* check for a semicolon *)
...
and parseProd toklist sc = match toklist with
  | [ ] -> raise (Error "Error: Incomplete expression")
  | l -> if (token_exists l L.TIMES) then
      Prod ( parseAtom ( list_before_token l L.TIMES ) sc, parseProd ( list_after_token l L.TIMES ) sc)
    else
      parseAtom l sc
...
and parseAtom toklist sc = match toklist with
  | [ ] -> raise (Error "Error: Incomplete expression")
  | x::xs -> match x with
    | L.LPAREN -> parseSum ( list_before_token xs L.RPAREN ) (* something with list_after_token xs L.RPAREN *)
    | L.INT(n) -> Int ( n )
    | _ -> raise (Error "Error: Improperly formed expression")

...
loopj:
	...
	sub.s	$f2, $f13, $f7		#$f2 = b = (3 / h ) * j - 1.5
loopi:
	addiu	$t1, $t1, 1		#i++
	mtc1	$t1, $f8		#copy value of i to FPU
	cvt.s.w	$f8, $f8		#convert $f8 to float
	
	lwc1	$f3, const4		#$f3 -> x0 = 0
	lwc1	$f4, const4		#$f4 -> y0 = 0
	
	mul.s	$f13, $f5, $f8		#$f13 = (3 / w) * i
	sub.s	$f1, $f13, $f10		#$f1 = a = (3 / w ) * i - 2.0
...
const4: .float 0.0		#I really have no clue how else to get an exact zero in the FPU
const5: .float 2.0		#floats are frustrating to do math with

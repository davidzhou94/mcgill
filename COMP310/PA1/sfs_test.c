...
char *rand_name() 
{
  ...
  for (i = 0; i < MAX_FNAME_LENGTH-1; i++) {
    if (i < 16) {
      ...
      fname[i] = (char)(((int)'0')+fname_count / 100);
    } else if (i == 18) {
      fname[i] = (char)(((int)'0')+fname_count / 10);
    } else {
      fname[i] = (char)(((int)'0')+fname_count%10);
    }
  }
...
main(int argc, char **argv)
{
  ...
  for (i = 0; i < 2; i++) {
    for (j = i + 1; j < 2; j++) {
      if (fds[i] == fds[j]) {
        fprintf(stderr, "Warning: the file descriptors (fds[%d]=%d,fds[%d]=%d) probably shouldn't be the same?\n",i,fds[i],j,fds[j]);
      }
    }
  }
  ...
  for (j = 0; j < strlen(test_str); j++) {
    for (i = 0; i < nopen; i++) {
      ...
      int ret = sfs_fread(fds[i], &ch, 1);
      if (ret != 1) {
        fprintf(stderr, "ERROR: Failed to read 1 character, received %d from file %d\n",ret,fds[i]);
        error_count++;
      } else {
        //printf("INFO: Read 1 char successfully from file %d\n",fds[i]);
      }
      if (ch != test_str[j]) {
  ...
  for (i = 0; i < nopen; i++) {
    ...
    if (fds[i] >= 0) {
      ...
      if (readsize != strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes in file %d (%s), received %d, expected %d\n",fds[i],names[i],readsize,(int)strlen(test_str));
        error_count++;
      } else {
        // fprintf(stderr, "INFO: Read succeeded on file %d (%s), received %d, expected %d\n",fds[i],names[i],readsize,(int)strlen(test_str));
	}
  ...
  for (i = 0; i < nopen; i++) {
    ...
    if (fds[i] >= 0) {
      readsize = sfs_fread(fds[i], fixedbuf, sizeof(fixedbuf));
      if (readsize < strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes, received %d, expected %d\n",readsize,(int)strlen(test_str));
        error_count++;
      }
